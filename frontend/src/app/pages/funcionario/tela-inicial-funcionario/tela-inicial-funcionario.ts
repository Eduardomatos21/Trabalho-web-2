...
export class TelaInicialFuncionario implements OnInit {
  ...
  // IGNORE
  // Este método é utilizado apenas para fins de testes
  private ignorarSolicitacao(codigo: string): void {
    this.solicitacoes = this.solicitacoes.filter((s) => s.codigo !== codigo);
  }

  // Este método é utilizado apenas para fins de testes
  private adicionarSolicitacao(solicitacao: SolicitacaoCliente): void {
    this.solicitacoes.push(solicitacao);
  }

  // Este método é utilizado apenas para fins de testes
  private atualizarSolicitacao(solicitacaoAtualizada: SolicitacaoCliente): void {
    this.solicitacoes = this.solicitacoes.map((s) => (s.codigo === solicitacaoAtualizada.codigo ? solicitacaoAtualizada : s));
  }

  // Este método é utilizado apenas para fins de testes
  private resetarSolicitacoes(): void {
    this.solicitacoes = this.carregarSolicitacoesFuncionario();
  } 

  private readonly solicitacoesBase2: SolicitacaoCliente[] = [
    {
      codigo: 'SOL-1042',
      nomeCliente: 'João da Silva',
      emailCliente: 'joao.silva@email.com',
